...
public class HttpResponseContentAssertionsSpecs
{
    ...
    public void When_asserting_response_with_no_content_it_should_succeed()
    {
        ...
        // Act
        Action act = () => subject.ShouldBeEmpty();
    ...
    public void When_asserting_response_with_content_it_should_throw_with_descriptive_message()
    {
        ...
        // Act
        Action act = () => subject.ShouldBeEmpty("we want to test the {0}", "reason");

        // Assert
        act.ShouldThrow<ShouldAssertException>()
            .Message.ShouldContain("to have no content");
    }
    ...
    [Fact]
    public void
        When_asserting_response_with_content_with_more_JSON_properties_than_a_model_to_be_as_model_it_should_succeed()
    {
        // Arrange
        using var subject = new HttpResponseMessage
        {
    ...
    [Fact]
    public void
        When_asserting_response_with_content_with_differences_to_be_as_model_it_should_throw_with_descriptive_message()
    {
        // Arrange
        using var subject = new HttpResponseMessage
        {
        ...
        // Assert
        act.ShouldThrow<ShouldAssertException>()
            .Message.ShouldMatch("(.*)Not Hey(.*)with a length(.*)reason(.*)");
    }
    ...
    public void When_asserting_response_with_content_as_primitives_types_to_be_as_the_primitive_value_it_should_succeed(
    ...
    [Fact]
    public void
        When_asserting_response_with_fewer_JSON_properties_than_the_model_to_be_as_model_it_should_throw_with_descriptive_message()
    {
        // Arrange
        using var subject = new HttpResponseMessage
        {
        ...
        // Assert
        act.ShouldThrow<ShouldAssertException>()
            .Message.ShouldMatch("""(.*)Author to be "John"(.*)reason(.*)""")
            ;
    }
    ...
    public void When_asserting_response_with_not_a_proper_JSON_to_be_as_model_it_should_throw_with_descriptive_message()
    {
        ...
        act.ShouldThrow<ShouldAssertException>()
            .Message.ShouldMatch(
                "(.*)to have a content equivalent to a model of type(.*), but the JSON representation could not be parsed(.*)")
            ;
    }

    [Fact]
    public void
        When_asserting_response_with_not_a_proper_JSON_to_be_as_model_it_should_throw_with_descriptive_message_which_includes_the_parsing_error_details()
    {
        // Arrange
        using var subject = new HttpResponseMessage
        {
        ...
        act.ShouldThrow<ShouldAssertException>()
            .Message.ShouldMatch(
                """(.*)to have a content equivalent to a model of type(.*), but the JSON representation could not be parsed, as the operation failed with the following message: "Exception while deserializing the model with SystemTextJsonSerializer: The JSON value could not be converted to (.*) Path: $.price | LineNumber: 0 | BytePositionInLine: 15.(.*)""")
            ;
    }

    [Fact]
    public void
        When_asserting_response_with_a_syntactically_correct_JSON_array_to_be_as_a_single_object_model_it_should_throw_with_descriptive_message_which_includes_the_serialization_error_details_but_keep_the_original_content()
    {
        // Arrange
        using var subject = new HttpResponseMessage
        {
            Content = new StringContent( /*lang=json,strict*/ """[{ "price" : 0.0}, { "price" : 1.0}]""", Encoding.UTF8,
                "application/json")
        };
...
//        Action act = () =>
//#if FAV8
//            subject.ShouldBeAs(new { }, options: (Func<EquivalencyOptions<object>, EquivalencyOptions<object>>)(null!));
//#else
//            subject.ShouldBeAs(new { }, options: (Func<EquivalencyAssertionOptions<object>, EquivalencyAssertionOptions<object>>)(null!));
//#endif

//        // Assert
//        act.ShouldThrow<ArgumentNullException>()
//            .Message.ShouldMatch("Value cannot be null(.*)options(.*)")
//            ;
//    }
    ...
    public void When_asserting_response_to_be_as_against_null_value_it_should_throw_with_descriptive_message()
    {
        ...
        // Assert
        act.ShouldThrow<ArgumentNullException>()
            .Message.ShouldMatch("(.*)content(.*)<null>(.*)")
            ;
    }
    ...
    public void When_asserting_null_response_to_be_as_it_should_throw_with_descriptive_message()
    {
        ...
        // Act
        Action act = () =>
            subject.ShouldBeAs((object?)null, "because we want to test the failure {0}", "message");

        // Assert
        act.ShouldThrow<ShouldAssertException>()
            .Message.ShouldMatch(
                "Expected a (.*) to assert because we want to test the failure message, but found <null>.")
            ;
    }
    ...
    //    // Act
    //    Action act = () =>
    //        subject.ShouldMatchInContent("(.*)notes(.*)", "because we want to test the failure {0}", "message");
    ...
    //    // Act
    //    Action act = () =>
    //        subject.ShouldMatchInContent("wildcard", "because we want to test the failure {0}", "message");
