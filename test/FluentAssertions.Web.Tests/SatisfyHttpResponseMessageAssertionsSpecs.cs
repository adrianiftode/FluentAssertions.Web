...
namespace FluentAssertions.Web.Tests
{
    public class SatisfyHttpResponseMessageAssertionsSpecs
    {
        ...
        public void When_asserting_response_without_having_satisfiable_assertion_to_satisfy_several_assertions_it_should_throw_with_descriptive_message()
        {
            ...
            // Assert
            act.Should().Throw<XunitException>()
                .WithMessage("*Expected value to satisfy one or more assertions, but it wasn't because we want to test the reason:*expected response.Headers.AcceptRanges {empty} to contain*- expected response.Headers to be <null>, but found {empty}*The HTTP response was:*");
        }
